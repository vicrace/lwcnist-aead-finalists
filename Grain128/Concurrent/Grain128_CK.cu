...
int main()
{
	...
	char writeFile[100];
	char fineLvl[1];
	strcpy(writeFile, "Grain_Concurent_F");
	...
	printf("Version\t\t\tKernel\tMemcpyH2D\tMemcpyD2H\tLatency\t\tAEAD/s (full latency)\t AEAD/s (exclude transpose)\n\n");
...
#ifdef WRITEFILE
	fprintf(fpt, "%s, %.6f, %.6f, %.6f,%.6f, %.6f, %.6f, %.6f, %.6f, %.f, %.2f\n", "Host Ref", 0, 0.0, cpu_t, 0.0, 0.0, cpu_t, 0.0, BATCH / (cpu_t / 1000), BATCH / (cpu_t / 1000));
#endif

	printf("Host Ref\t\tSerial\t%.6f\t%.6f\t%.6f\t%.f\n", 0.0, 0.0,  cpu_t, BATCH / (cpu_t / 1000));
		...
		if (z != 3) {
			...
			for (int i = 64; i < 1025; i *= 2) {
				...
				for (int a = 1; a < 5; a++) {
					...
					kernel = ((a == 1) ? &grain_encrypt_ref_GPU : ((a == 2) ? &grain_encrypt_MemOp_GPU
						: ((a == 3) ? &grain_encrypt_ref_GPU_Trans : &grain_encrypt_MemOp_GPU_Trans)));

					char* kernelName = ((a == 1) ? "GPU Ref " : ((a == 2) ? "Mem Op" : ((a == 3) ? "Ref Trans " : "MemOp Trans")));
					...
					printf("%s %d\t\t%d\t%.6f\t%.6f\t%.6f\t%.f \t%.f\n", kernelName, threads.x, z, memcpy_h2d,
			...
			for (int i = 0; i < z; ++i) {
				int ioffset = i * iBATCH;
				grain_encrypt_FineOp_GPU << <blocks2, threads2, 0, GPUstreams[i] >> > (&d_c[ioffset * MAX_CIPHER_LENGTH], d_clen, &d_m[ioffset * mlen], mlen, &d_a[ioffset * alen], alen, 0,
					&d_n[ioffset * CRYPTO_NPUBBYTES], &d_k[ioffset * CRYPTO_KEYBYTES]);
			}
			cudaEventRecord(stop);
			cudaEventSynchronize(stop);
			...
			for (int i = 0; i < z; ++i) {
				int ioffset = i * iBATCH;
				cudaMemcpyAsync(&h_c[ioffset * MAX_CIPHER_LENGTH], &d_c[ioffset * MAX_CIPHER_LENGTH], size, cudaMemcpyDeviceToHost, GPUstreams[i]);
			}
			cudaEventRecord(stop, 0);
			cudaEventSynchronize(stop);
...
#ifdef WRITEFILE
			fprintf(fpt, "%s%d, %d, (%u)(%u), %.6f, %.6f, %.6f, %.6f,%.6f, %.6f, %.6f, %.6f, %.f,%.2f\n", "Fine ", fineLevel, z, blocks2.x, threads2.x,(size * 2e-6) / total, total,
