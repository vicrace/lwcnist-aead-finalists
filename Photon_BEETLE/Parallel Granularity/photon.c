...
__device__ void PHOTON_PermutationG_Fine(unsigned char* State_in)
{
	__shared__ byte state[D][D];
	int i;

	double innertid = (double)threadIdx.x / (double)fineLevel;
	double a = (innertid * fineLevel * ceil(((double)(D * D) / (double)fineLevel)));
	double b = (a + ceil(((double)(D * D) / (double)fineLevel)));
	b = (b > (D * D)) ? D * D : b; //prevent overload
	...
	int z;
	for (z = 0; z < ROUND; z++) {
		AddKeyG(state, z); //PrintStateG(state);

		int c, r;
		tword v;
		__shared__ byte os[D][D];
		memcpy(os, state, D * D);

		innertid = (double)threadIdx.x / (double)fineLevel;
		a = (innertid * fineLevel * ceil(((double)D / (double)fineLevel)));
		b = (a + ceil(((double)D / (double)fineLevel)));
		b = (b > D) ? D : b; //prevent overload

		//printf ("tid -> %d, innertid -> %.6f, a -> %.6f , b -> %.6f\n", threadIdx.x, innertid, a, b);

		for (c = a; c < b; c++) { // for all columns
			v = 0;
			v ^= TableG[os[0][(0 + c) % D]];
			v ^= TableG[1 * 16 + os[1][(1 + c) % D]];
			v ^= TableG[2 * 16 + os[2][(2 + c) % D]];
			v ^= TableG[3 * 16 + os[3][(3 + c) % D]];
			v ^= TableG[4 * 16 + os[4][(4 + c) % D]];
			v ^= TableG[5 * 16 + os[5][(5 + c) % D]];
			v ^= TableG[6 * 16 + os[6][(6 + c) % D]];
			v ^= TableG[7 * 16 + os[7][(7 + c) % D]];
			state[D - 1][c] = (uint8_t)v & WORDFILTER;      v >>= S;
			state[D - 2][c] = (uint8_t)v & WORDFILTER;      v >>= S;
			state[D - 3][c] = (uint8_t)v & WORDFILTER;      v >>= S;
			state[D - 4][c] = (uint8_t)v & WORDFILTER;      v >>= S;
