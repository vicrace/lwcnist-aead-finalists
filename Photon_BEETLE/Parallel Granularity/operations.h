...
static void checkResult(char* version, uint8_t* host, uint8_t* gpu, uint64_t clen, int batch, int t) {

	bool check = true;
	uint64_t i;
	for (int k = 0; k < batch; k++) {
		for (i = 0; i < clen; ++i) {
			if (host[k * clen + i] != gpu[k * clen + i] && t > 64) {
...
static void PrintTime(uint8_t* ct, uint8_t* h_c, uint64_t* d_clen, int threads, float memcpyH2D, float kernelTime, float memcpyD2H, float cpu_t, FILE * fpt, int batch, char* name) {
	...
	printf("KernelT%d :\t\t%.6f ms \t%.6f ms\t\t %.6f \t%.f\t%s\n", threads, memcpyH2D, memcpyD2H,  total, batch / (total / 1000), name);
#ifdef WRITEFILE
