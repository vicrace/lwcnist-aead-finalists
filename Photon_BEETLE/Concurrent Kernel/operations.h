...
static void PrintTime(uint8_t* ct, uint8_t* h_c, uint64_t* d_clen, int threads, float memcpyH2D, float kernelTime, float memcpyD2H, float cpu_t, FILE * fpt) {
	...
	printf("KernelT%d :\t %.6f ms\t(%f GB/s)\t\t %.6f ms \t(%f GB/s)\t %.2f times \n", threads, kernelTime, (size * 1e-6) / kernelTime, total, (size * 2e-6) / total, (cpu_t / total));
	fprintf(fpt, "T%d,%d,%.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f\n", threads, BATCH, (size * 2e-6) / total, total, (cpu_t / total), ((size * 2e-6) / total) * 8, memcpyH2D, kernelTime, (cpu_t/kernelTime), BATCH/total);
