...
//GPU
__device__ static inline uint64_t LOADBYTESG(const uint8_t * bytes, int n) {
	uint64_t x = 0;
	for (int i = 0; i < n; ++i) //printf("%x",bytes[i]);
		x |= SETBYTE(bytes[i], i);
...
__device__ static inline void STOREBYTESG(uint8_t * bytes, uint64_t x, int n) {
	for (int i = 0; i < n; ++i) bytes[i] = GETBYTE(x, i);
}

__device__ static inline uint64_t CLEARBYTESG(uint64_t x, int n) {
	for (int i = 0; i < n; ++i) x &= ~SETBYTE(0xff, i);
	return x;
}
