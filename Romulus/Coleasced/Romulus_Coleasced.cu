...
int main()
{
...
#ifdef WRITEFILE
	fprintf(fpt, "%s, %.6f, %.6f, %.6f,%.6f, %.6f, %.6f, %.6f, %.6f, %.f, %.2f\n", name, 0.0, cpu_t, 0.0, 0.0, cpu_t, 0.0, BATCH / (cpu_t / 1000), BATCH / (cpu_t / 1000));
#endif
	...
	for (int i = 64; i < 1025; i *= 2) {
		...
		for (int a = 1; a < 5; a++) {
			...
			else if ((a == 3 && ROMULUS_VER == "T") || (a == 4 && ROMULUS_VER != "T")) { // for coleasced
				cudaEventRecord(start, 0);
			...
			if (a > 2) {
				...
				blocks.x = ((a == 3) ? ceil(temp / 4) : ceil(temp)); // at least 1 block, if unroll then div 4, else remain
