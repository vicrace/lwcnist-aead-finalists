...
#ifdef ___SKINNY_LOOP
	RunEncryptionKeyScheduleTK3(pskinny_ctrl->roundKeys, RCO);
#else
...
void padO(const unsigned char* m, unsigned char* mp, int len8) {
...
void g8AO(unsigned char* s, unsigned char* c) {
...
void reset_lfsr_gf56O(unsigned char* CNT) {
...
void lfsr_gf56O(unsigned char* CNT) {
...
void nonce_encryptionO(
	...
	block_cipherO(s, k, (unsigned char*)N, CNT, D, p_skinny_ctrl);
...
unsigned long long msg_encryptionO(
	...
	*M = *M + len8;
	lfsr_gf56O(CNT);
	if (mlen != 0) {
		nonce_encryptionO(N, CNT, s, k, D, l_skinny_ctrl);
	}
	return mlen;

}

unsigned long long ad2msg_encryptionO(
	const unsigned char** M,
	unsigned char* CNT,
	unsigned char* s, const unsigned char* k,
	unsigned char D,
	...
	padO(*M, T, len8);
	block_cipherO(s, k, T, CNT, D, l_skinny_ctrl);
	lfsr_gf56O(CNT);
	*M = *M + len8;
...
unsigned long long ad_encryptionO(
...
unsigned long long msg_encryption_eqov16(
	...
	*M = *M + 16;
	lfsr_gf56O(CNT);
	nonce_encryptionO(N, CNT, s, k, D, p_skinny_ctrl);
	return mlen - 16;
...
unsigned long long ad_encryption_eqov32(
	...
	* A = *A + 16;
	block_cipherO(s, k, T, CNT, D, p_skinny_ctrl);
	lfsr_gf56O(CNT);
