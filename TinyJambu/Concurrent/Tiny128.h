...
__device__ void state_update_OpRef_Register_Trans(unsigned int* state, const unsigned char* key, unsigned int number_of_steps)
{
	unsigned int i;
	unsigned int t1, t2, t3, t4, feedback;
	unsigned int state0 = state[0], state1 = state[1], state2 = state[2], state3 = state[3];
...
#pragma unroll
	for (i = 0; i < number_of_steps; i = i + 128)
	{
		...
		t3 = (state2 >> 21) | (state3 << 11);  // 47 + 23 + 15 = 85 = 2*32 + 21      
		t4 = (state2 >> 27) | (state3 << 5);   // 47 + 23 + 15 + 6 = 91 = 2*32 + 27 
		state0 ^= t1 ^ (~(t2 & t3)) ^ t4 ^ ((unsigned int*)key)[0];

		t1 = (state2 >> 15) | (state3 << 17);
		t2 = (state3 >> 6) | (state0 << 26);
		t3 = (state3 >> 21) | (state0 << 11);
		t4 = (state3 >> 27) | (state0 << 5);
		state1 ^= t1 ^ (~(t2 & t3)) ^ t4 ^ ((unsigned int*)key)[1];

		t1 = (state3 >> 15) | (state0 << 17);
		t2 = (state0 >> 6) | (state1 << 26);
		t3 = (state0 >> 21) | (state1 << 11);
		t4 = (state0 >> 27) | (state1 << 5);
		state2 ^= t1 ^ (~(t2 & t3)) ^ t4 ^ ((unsigned int*)key)[2];

		t1 = (state0 >> 15) | (state1 << 17);
		t2 = (state1 >> 6) | (state2 << 26);
		t3 = (state1 >> 21) | (state2 << 11);
		t4 = (state1 >> 27) | (state2 << 5);
		state3 ^= t1 ^ (~(t2 & t3)) ^ t4 ^ ((unsigned int*)key)[3];
	}

	state[0] = state0, state[1] = state1, state[2] = state2, state[3] = state3;
}

__device__ void initialization_GPU_Op_Trans(const unsigned char* K, const unsigned char* iv, unsigned int* state)
{
	int i;
...
__device__ void process_ad_GPU_Op_Trans(const unsigned char* k, const unsigned char* ad, unsigned long long adlen, unsigned int* state)
{
	unsigned long long i;
	unsigned int j;
