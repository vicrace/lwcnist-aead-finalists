...
static void PrintTime(uint8_t* ct, uint8_t* h_c, uint64_t* d_clen, int threads, float memcpyH2D, float kernelTime, float memcpyD2H, float cpu_t, FILE * fpt, int BATCH, char* name, const int len) {
	...
	size_t size = BATCH * MAX_CIPHER_LENGTH(len) * sizeof(uint8_t);
	float total = memcpyH2D + kernelTime + memcpyD2H;

	if (strstr(name, "MemOp") != NULL) {
		printf("KernelT%d :\t\t %.6f ms\t %.2f times\t%.f \t %s\n", threads, total, (cpu_t / total), BATCH / (total / 1000),  name);
	}
