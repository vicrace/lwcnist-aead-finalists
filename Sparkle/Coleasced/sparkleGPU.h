...
// Round constants
__device__ static const uint32_t RCONG[MAX_BRANCHES] = { \
  0xB7E15162, 0xBF715880, 0x38B4DA56, 0x324E7738, \
  0xBB1185EB, 0x4F7C7B57, 0xCFBFA1C8, 0xC2B3293D  \
};

__device__ void linear_layer_GPU(SparkleState* state, int brans)
{
	int i, b = brans / 2;
...
__device__ void sparkle_ref_GPU(SparkleState* state, int brans, int steps)
{
	...
	for (i = 0; i < steps; i++) {
		// Add step counter
		state->y[0] ^= RCONG[i % MAX_BRANCHES];
		state->y[1] ^= i;
		// ARXBox layer
		for (j = 0; j < brans; j++)
			ARXBOX(state->x[j], state->y[j], RCONG[j]);
		// Linear layer
		linear_layer_GPU(state, brans);
	}
}
...
__device__ static void rho_whi_aut_GPU(SparkleState * state, const uint8_t * in, size_t inlen)
{
...
__device__ static void rho_whi_enc_GPU(SparkleState * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
...
__device__ static void rho_whi_dec_GPU(SparkleState * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	uint32_t inbuf[RATE_WORDS] = { 0 }, outbuf[RATE_WORDS];
...
__device__ void Initialize_GPU(SparkleState * state, const uint8_t * key, const uint8_t * nonce)
{
	...
	sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void ProcessAssocData_GPU(SparkleState * state, const uint8_t * in, size_t inlen)
{
	// Main Authentication Loop

	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_aut_GPU(state, in, RATE_BYTES);
		// execute SPARKLE with slim number of steps
		sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		in += RATE_BYTES;
	}
	...
	// combined Rho and rate-whitening operation
	rho_whi_aut_GPU(state, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void ProcessPlainText_GPU(SparkleState * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	...
	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_enc_GPU(state, out, in, RATE_BYTES);
		// execute SPARKLE with slim number of steps
		sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		out += RATE_BYTES;
	...
	rho_whi_enc_GPU(state, out, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void Finalize_GPU(SparkleState * state, const uint8_t * key)
{
...
__device__ void GenerateTag_GPU(SparkleState * state, uint8_t * tag)
{
...
__device__ void ProcessCipherText_GPU(SparkleState * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	...
	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_dec_GPU(state, out, in, RATE_BYTES);
		// execute SPARKLE with slim number of steps
		sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		out += RATE_BYTES;
	...
	state->y[STATE_BRANS - 1] ^= ((inlen < RATE_BYTES) ? CONST_M2 : CONST_M3);
	// combined Rho and rate-whitening (incl. padding)
	rho_whi_dec_GPU(state, out, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_ref_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void sparkle_opt_GPU(uint32_t * state, int brans, int steps)
{
	int i, j;  // Step and branch counter
...
__device__ static void rho_whi_aut_Op_GPU(uint32_t * state, const uint8_t * in, int aligned)
{
...
__device__ static void rho_whi_aut_last_Op_GPU(uint32_t * state, const uint8_t * in, size_t inlen)
{
	uint32_t buffer[RATE_WORDS];
	uint8_t* bufptr;
	uint32_t tmp;
...
__device__ static void rho_whi_enc_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, \
	int aligned)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t* in32, * out32;
...
__device__ static void rho_whi_enc_last_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, size_t inlen)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t tmp1, tmp2;
	uint8_t* bufptr;
...
__device__ static void rho_whi_dec_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, \
	int aligned)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t* in32, * out32;
...
__device__ static void rho_whi_dec_last_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t tmp1, tmp2;
...
__device__ void Initialize_Op_GPU(uint32_t * state, const uint8_t * key, const uint8_t * nonce)
{
	// load nonce into the rate-part of the state
	...
	// execute SPARKLE with big number of steps
	sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}

__device__ void ProcessAssocData_Op_GPU(uint32_t * state, const uint8_t * in, size_t inlen)
{
	...
	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_aut_Op_GPU(state, in, aligned);
		// execute SPARKLE with slim number of steps
		sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		in += RATE_BYTES;
	}
	...
	// combined Rho and rate-whitening (incl. padding)
	rho_whi_aut_last_Op_GPU(state, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}

__device__ void ProcessPlainText_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	...
	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_enc_Op_GPU(state, out, in, aligned);
		// execute SPARKLE with slim number of steps
		sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		out += RATE_BYTES;
	...
	// combined Rho and rate-whitening (incl. padding)
	rho_whi_enc_last_Op_GPU(state, out, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}

__device__ void Finalize_Op_GPU(uint32_t * state, const uint8_t * key)
{
...
__device__ void GenerateTag_Op_GPU(uint32_t * state, uint8_t * tag)
{
	memcpy(tag, (state + RATE_WORDS), TAG_BYTES);
}

__device__ void ProcessCipherText_Op_GPU(uint32_t * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	// check whether 'in' and 'out' can be casted to uint32_t pointer
	int aligned = (((size_t)in) | ((size_t)out)) % UI32_ALIGN_BYTES == 0;
	...
	while (inlen > RATE_BYTES) {
		// combined Rho and rate-whitening operation
		rho_whi_dec_Op_GPU(state, out, in, aligned);
		// execute SPARKLE with slim number of steps
		sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
		inlen -= RATE_BYTES;
		out += RATE_BYTES;
	...
	state[STATE_WORDS - 1] ^= ((inlen < RATE_BYTES) ? CONST_M2 : CONST_M3);
	// combined Rho and rate-whitening (incl. padding)
	rho_whi_dec_last_Op_GPU(state, out, in, inlen);
	// execute SPARKLE with big number of steps
	sparkle_opt_GPU(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
__device__ void sparkle_opt_GPU_Op(uint32_t * state, int brans, int steps)
{
	int i, j;  // Step and branch counter
	uint32_t rc, tmpx, tmpy, x0, y0;
...
__device__ static void rho_whi_aut_Op_GPU_Op(uint32_t * state, const uint8_t * in, int aligned)
{
	uint32_t buffer[RATE_WORDS]; //optimised share memory
	uint32_t* in32;
	uint32_t tmp;
	int i, j;
	...
	//optimised UNROLL
	for (i = 0, j = RATE_WORDS / 2; i < RATE_WORDS / 2; i += 2, j += 2) {
		tmp = state[i];
		state[i] = state[j] ^ in32[i] ^ state[RATE_WORDS + i];
		state[j] ^= tmp ^ in32[j] ^ state[RATE_WORDS + CAP_INDEX(j)];
		...
		//unroll 2
		tmp = state[i + 1];
		state[i + 1] = state[j + 1] ^ in32[i + 1] ^ state[RATE_WORDS + (i + 1)];
		state[j + 1] ^= tmp ^ in32[j + 1] ^ state[RATE_WORDS + CAP_INDEX(j + 1)];
	}
}
...
__device__ static void rho_whi_aut_last_Op_GPU_Op(uint32_t * state, const uint8_t * in, size_t inlen)
{
	uint32_t buffer[RATE_WORDS];
	uint8_t* bufptr;
	uint32_t tmp;
	...
	for (i = 0, j = RATE_WORDS / 2; i < RATE_WORDS / 2; i += 2, j += 2) {
		...
		//unroll 2
		tmp = state[i + 1];
		state[i + 1] = state[j + 1] ^ buffer[i + 1] ^ state[RATE_WORDS + (i + 1)];
		state[j + 1] ^= tmp ^ buffer[j + 1] ^ state[RATE_WORDS + CAP_INDEX(j + 1)];
	}
}
...
__device__ static void rho_whi_enc_Op_GPU_Op(uint32_t * state, uint8_t * out, const uint8_t * in, \
	int aligned)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t* in32, * out32;
	...
	for (i = 0, j = RATE_WORDS / 2; i < RATE_WORDS / 2; i ++, j++) {
		...
		tmp2 = state[j + 1];
		state[i + 1] = state[j + 1] ^ in32[i + 1] ^ state[RATE_WORDS + (i + 1)];
		state[j + 1] ^= tmp1 ^ in32[j + 1] ^ state[RATE_WORDS + CAP_INDEX(j)];
		out32[i + 1] = in32[i + 1] ^ tmp1;
		out32[j + 1] = in32[j + 1] ^ tmp2;*/
	}
...
__device__ static void rho_whi_enc_last_Op_GPU_Op(uint32_t * state, uint8_t * out, const uint8_t * in, size_t inlen)
{
	uint32_t buffer[RATE_WORDS];
	uint32_t tmp1, tmp2;
	uint8_t* bufptr;
...
__device__ static void rho_whi_dec_Op_GPU_Op(uint32_t * state, uint8_t * out, const uint8_t * in, \
	int aligned)
{
	__shared__ uint32_t buffer[RATE_WORDS];		//share memory optimised
	uint32_t* in32, * out32;
	uint32_t tmp1, tmp2;
	int i, j;
	...
	for (i = 0, j = RATE_WORDS / 2; i < RATE_WORDS / 2; i += 2, j += 2) {
		...
		tmp2 = state[j + 1];
		state[i + 1] ^= state[j + 1] ^ in32[i + 1] ^ state[RATE_WORDS + i + 1];
		state[j + 1] = tmp1 ^ in32[j + 1] ^ state[RATE_WORDS + CAP_INDEX(j + 1)];
		out32[i + 1] = in32[i + 1] ^ tmp1;
		out32[j + 1] = in32[j + 1] ^ tmp2;
	}
...
__device__ static void rho_whi_dec_last_Op_GPU_Op(uint32_t * state, uint8_t * out, const uint8_t * in, \
	size_t inlen)
{
	__shared__ uint32_t buffer[RATE_WORDS]; //shared memory here - optimised
	uint32_t tmp1, tmp2;
	uint8_t* bufptr;
	int i, j;
	...
	for (i = 0, j = RATE_WORDS / 2; i < RATE_WORDS / 2; i += 2, j += 2) {
		...
		tmp1 = state[i + 1];
		tmp2 = state[j + 1];
		state[i + 1] ^= state[j] ^ buffer[i + 1] ^ state[RATE_WORDS + i + 1];
		state[j + 1] = tmp1 ^ buffer[j + 1] ^ state[RATE_WORDS + CAP_INDEX(j + 1)];
		buffer[i + 1] ^= tmp1;
		buffer[j + 1] ^= tmp2;
	}
	memcpy(out, buffer, inlen);
...
//__device__ void Initialize_Op_GPU_Op(uint32_t* state, const uint8_t* key, const uint8_t* nonce)
//{
//	// load nonce into the rate-part of the state
//	memcpy(state, nonce, NONCE_BYTES);
//	// load key into the capacity-part of the sate
...
//}
//
//__device__ void ProcessAssocData_Op_GPU_Op(uint32_t* state, const uint8_t* in, size_t inlen)
//{
//	// check whether 'in' can be casted to uint32_t pointer
//	int aligned = ((size_t)in) % UI32_ALIGN_BYTES == 0;
//	// printf("Address of 'in': %p\n", in);
...
//	while (inlen > RATE_BYTES) {
//		// combined Rho and rate-whitening operation
//		rho_whi_aut_Op_GPU_Op(state, in, aligned);
//		// execute SPARKLE with slim number of steps
...
//	state[STATE_WORDS - 1] ^= ((inlen < RATE_BYTES) ? CONST_A0 : CONST_A1);
//	// combined Rho and rate-whitening (incl. padding)
//	rho_whi_aut_last_Op_GPU_Op(state, in, inlen);
//	// execute SPARKLE with big number of steps
//	sparkle_opt_GPU_Op(state, STATE_BRANS, SPARKLE_STEPS_BIG);
//}
//
//__device__ void ProcessPlainText_Op_GPU_Op(uint32_t* state, uint8_t* out, const uint8_t* in, \
//	size_t inlen)
//{
//	// check whether 'in' and 'out' can be casted to uint32_t pointer
//	int aligned = (((size_t)in) | ((size_t)out)) % UI32_ALIGN_BYTES == 0;
...
//	while (inlen > RATE_BYTES) {
//		// combined Rho and rate-whitening operation
//		rho_whi_enc_Op_GPU_Op(state, out, in, aligned);
//		// execute SPARKLE with slim number of steps
//		sparkle_opt_GPU_Op(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
//		inlen -= RATE_BYTES;
//		out += RATE_BYTES;
...
//	state[STATE_WORDS - 1] ^= ((inlen < RATE_BYTES) ? CONST_M2 : CONST_M3);
//	// combined Rho and rate-whitening (incl. padding)
//	rho_whi_enc_last_Op_GPU_Op(state, out, in, inlen);
//	// execute SPARKLE with big number of steps
//	sparkle_opt_GPU_Op(state, STATE_BRANS, SPARKLE_STEPS_BIG);
//}
//
//__device__ void Finalize_Op_GPU_Op(uint32_t* state, const uint8_t* key)
//{
//	uint32_t buffer[TAG_WORDS];
//	int i;
//
...
//// The GenerateTag function generates an authentication tag.
//
//__device__ void GenerateTag_Op_GPU_Op(uint32_t* state, uint8_t* tag)
//{
//	memcpy(tag, (state + RATE_WORDS), TAG_BYTES);
//}
//
//__device__ void ProcessCipherText_Op_GPU_Op(uint32_t* state, uint8_t* out, const uint8_t* in, \
//	size_t inlen)
//{
//	// check whether 'in' and 'out' can be casted to uint32_t pointer
//	int aligned = (((size_t)in) | ((size_t)out)) % UI32_ALIGN_BYTES == 0;
...
//	while (inlen > RATE_BYTES) {
//		// combined Rho and rate-whitening operation
//		rho_whi_dec_Op_GPU_Op(state, out, in, aligned);
//		// execute SPARKLE with slim number of steps
//		sparkle_opt_GPU_Op(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
//		inlen -= RATE_BYTES;
//		out += RATE_BYTES;
...
//	state[STATE_WORDS - 1] ^= ((inlen < RATE_BYTES) ? CONST_M2 : CONST_M3);
//	// combined Rho and rate-whitening (incl. padding)
//	rho_whi_dec_last_Op_GPU_Op(state, out, in, inlen);
//	// execute SPARKLE with big number of steps
//	sparkle_opt_GPU_Op(state, STATE_BRANS, SPARKLE_STEPS_BIG);
//}